class BookmarksController < ApplicationController
  ...
  def create
    ...
    the_bookmark.recipe_id = params.fetch("query_recipe_id")

    if the_bookmark.valid?
      the_bookmark.save
      redirect_to("/recipes/#{the_bookmark.recipe_id}", { :notice => "Bookmark created successfully." })
    else
      redirect_to("/recipes/#{the_bookmark.recipe_id}", { :notice => "Bookmark failed to create successfully." })
    end
  end
